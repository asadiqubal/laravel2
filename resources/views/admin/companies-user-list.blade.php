...
@section('title', 'Company User List')
@section("content")
      ...
      <div class="content-wrapper">
        <div class="content-header row">
			  <div class="content-header-left col-md-6 col-12 mb-1">
				<h3 class="content-header-title">Companies User List</h3>
			  </div>
			  <div class="content-header-right breadcrumbs-right breadcrumbs-top col-md-6 col-12">
				<div class="breadcrumb-wrapper col-12">
				  <ol class="breadcrumb">
					...
					<li class="breadcrumb-item active">Companies User List
					</li>
				  </ol>
				</div>
			  </div>
					...
					<!--<a class="btn btn-info" href="{{action('AdminController@export')}}">Export</a>-->
					   <div class="row mb-2 justify-content-center">
					         <div class="col-md-4 col-12">
								<form action="{{url('admin/company-user-list')}}" method="GET">

									<select class="form-control" id="countrySelect" name="company"  onchange="this.form.submit()">
										<option value="" selected="">Select Company</option>
										@if($comapany_list)
											@foreach($comapany_list as $eachitem)
												<option <?php if(isset($_GET['company']) && $_GET['company'] == $eachitem['id']){ echo "selected"; } ?> value="{{$eachitem['id']}}">{{$eachitem['name']}}</option>
											@endforeach
										@endif
									</select>
								</form>
                            ...
                            <tbody>
								@if(count($company_user_list) > 0)
                                    ...
                                    <tr>
										...
										?>
                                        <td> <a href="{{url('/admin/edit-company-users')}}/{{$eachitem['id']}}">Edit</a> | <a  onclick="deleteFunc('User',{{$eachitem['id']}})" href="javascript:void(0)">Delete</a></td>
                                    </tr>
                                    @endforeach
                                @endif
