...
<style>
  ...
  <body class="vertical-layout vertical-menu 2-columns   menu-expanded fixed-navbar" data-open="click" data-menu="vertical-menu" data-col="2-columns">
    ...
    @include("partial.supplier.header")
	  @include("partial.supplier.sidemenu")
  ...
  function deleteFunccom(model,id){
    if(confirm("All data related to this company will be deleted, including users, ad RFQs etc.")){
      window.location.href = "{{ url('/supplier/deleteByID') }}/"+model+'/'+id;
    }
    else{
      return false;
    }
  ...
  function updateByID(model,id){
    ...
    if(value){
      window.location.href = "{{ url('/supplier/updateByID') }}/"+model+'/'+id+'/'+value;
    }
    else{
